      ...
      <div className="bg-[#0a0a0e] border border-white/10 w-full max-w-5xl rounded-[30px] sm:rounded-[40px] p-5 sm:p-8 relative overflow-hidden shadow-2xl flex flex-col md:flex-row gap-6 sm:gap-8 my-auto">
        ...
        <div className="md:w-2/3 grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 relative z-10 pb-4 md:pb-0">
          {PRICING_PLANS.map((plan) => (
            <div 
              key={plan.id}
              className={`relative flex flex-col p-5 sm:p-6 rounded-[24px] sm:rounded-[32px] border transition-all duration-300 group hover:scale-[1.02] cursor-pointer ${
                plan.popular 
                ? 'bg-[#151925] border-[#ccff00]/30 shadow-[0_15px_30px_rgba(0,0,0,0.4)]' 
                : 'bg-[#0f1119] border-white/5 hover:border-white/10'
              }`}
              onClick={() => onPurchase(plan.id as PlanType, plan.tokens)}
            >
              {plan.popular && (
                <div className="absolute -top-2.5 left-1/2 -translate-x-1/2 bg-[#ccff00] text-black text-[8px] font-black uppercase tracking-widest px-3 py-1 rounded-full shadow-lg z-20">
                  Best Value
                </div>
              )}
              
              <div className="flex-1 space-y-3 sm:space-y-4">
                <div className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">{plan.name}</div>
                <div className="flex flex-col">
                  <span className="text-2xl sm:text-3xl font-black text-white">{plan.price}</span>
                  <span className="text-[9px] text-slate-500 font-bold mt-0.5 uppercase tracking-wide">Secure Checkout</span>
                </div>
                
                <div className="pt-3 sm:pt-4 space-y-2 sm:space-y-3">
                  <div className="flex items-center gap-2 text-[#ccff00]">
                    <Zap size={14} fill="currentColor" />
                    <span className="text-base sm:text-lg font-black">{plan.tokens} Edits</span>
                  </div>
                  <div className="text-[9px] text-slate-500 font-bold uppercase tracking-wider">Never expires</div>
                </div>
              </div>

              <button className={`w-full mt-5 sm:mt-6 py-3 rounded-xl sm:rounded-2xl font-black text-xs sm:text-sm transition-all shadow-md ${
                plan.popular 
